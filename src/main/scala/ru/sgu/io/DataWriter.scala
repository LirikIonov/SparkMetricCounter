...
class DataWriter(implicit config: SparkMetricConfig) extends Serializable with Logging {
  def writeToStorage(bikeRDD: DataFrame): Unit = {
    ...
    logger.info("Saving bike rdd into Storage...")
  ...
  def writeToBigQuery(bikeFrame: DataFrame, tableName: String): Unit = {
    logger.info(s"Saving into $tableName (BigQuery)")
    //bikeFrame.show(10)
